...
def get_lazada_data():
    ...
    url = " https://dienthoaivui.com.vn/tim-kiem?_tim_kiem=điện%20thoại"

    driver.get(url)

    # Lấy danh sách các thẻ sản phẩm (mỗi sản phẩm nằm trong một div có class .Bm3ON)
    products = driver.find_elements(By.CSS_SELECTOR, ".w-full")  # Lấy danh sách sản phẩm
    ...
    for product in products[:10]:  # Giới hạn lấy 10 sản phẩm đầu
        try:
            ...
            title = product.find_element(By.CSS_SELECTOR, ".name-product    ").text
